...
const TipoPage = () => {
    ...
    const handleToggleEstado = async (tipo) => {
        ...
        if (!window.confirm(`¿Estás seguro de que quieres ${mensajeConfirmacion} la Marca "${tipo.nombreTipo}"?`)) return;

        try {
            await toggleTipoEstado(tipo.idTipo, nuevoEstado);
            alert(`Tipo "${tipo.nombreTipo}" ${mensajeAccion} con éxito.`);
    ...
    const handleDelete = async (id, nombreTipo) => {
        if (!window.confirm(`¿Estás seguro de que quieres eliminar el tipo "${nombreTipo}"? Esta acción es irreversible.`)) return;

        try {
            await deleteTipo(id);
            alert(`Tipo "${nombreTipo}" eliminada con éxito.`);
            await fetchData(paginaActual);
        } catch (err) {
                        ...
                        {cargando ? (
                            ...
                            tipos.map((tipo) => (
                                <tr key={tipo.idTipo}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tipo.nombreTipo}</td>
                               ...
                               bg-gray-800 text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100">
                                        ...
                                        <button
                                            ...
                                            className={`relative group ${tipo.estado ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'
                                                }`}
                                        >
                                            {tipo.estado ? (
                                        ...
                                        {/* ELIMINAR */}
                                        <button
                                            onClick={() => handleDelete(tipo.idTipo, tipo.nombreTipo)}
